...
### ⚠️ React 18 is an unsupported configuration

[Microsoft's SPFx compatibility matrix](https://learn.microsoft.com/en-us/sharepoint/dev/spfx/compatibility)
pins every released SPFx version — 1.23.0 included, the newest at the time
this was written — to **React 17.0.1 exactly**. No shipped SPFx release
supports React 18. This solution runs React 18 anyway, as a deliberate,
informed choice, and that choice comes with obligations for anyone touching
this code:

- `npm install` needs `--legacy-peer-deps`. `@microsoft/sp-core-library` and
  `@microsoft/sp-webpart-base` declare a hard peer dependency on
  `react <18.0.0` and simply refuse to resolve otherwise.
- Because that peer dependency goes unsatisfied, those SPFx packages pull in
  their own private nested copies of React 17 for internal use (the property
  pane machinery, the component loader). That's fine as implemented here:
  `config/config.json` sets `"externals": {}`, so our React 18 gets bundled
  entirely inside the web part's own JS file, and nothing in this app passes
  React elements or hooks across the boundary into an SPFx-provided React API
  — the property pane fields in use (`PropertyPaneLabel`, `PropertyPaneButton`)
  are plain declarative config objects, not JSX. If a future change reaches
  for an SPFx API that *does* accept React nodes — a Dialog/Panel helper, a
  Dynamic Data React binding, an extension surface — stop and re-evaluate
  first. That's exactly the scenario Microsoft's own "silent runtime
  failures" warning is describing.
- The web part bootstrap in `ProvisioningCenterWebPart.ts` uses the React 18
  `createRoot` / `root.unmount()` API, deliberately not the legacy
  `ReactDom.render`. Using the old API would quietly fall back to React 17
  compatibility mode and undo the point of upgrading in the first place.
- `scheduler` is pinned as a direct dependency purely so that
  `@fluentui/react-context-selector` can resolve it through normal node
  resolution — without it sitting at the root, webpack fails to bundle.
- **This is build- and test-verified, not tenant-verified.** Install,
  typecheck, lint, the unit test suite (126/126 at last count) and packaging
  all pass cleanly. Duplicate-React-instance bugs are the textbook case that
  never shows up locally and only appears on a real SharePoint page — verify
  there before trusting this in production, not just in the workbench.
- **To roll back:** pin `react` / `react-dom` back to `17.0.1` and
  `@types/react` / `@types/react-dom` back to `17.0.45` / `17.0.17` in both
  `package.json` and the `resolutions` block, drop the `scheduler`
  dependency, revert `ProvisioningCenterWebPart.ts` to
  `ReactDom.render(element, this.domElement)` /
  `ReactDom.unmountComponentAtNode(this.domElement)`, then run
  `npm install --legacy-peer-deps` again.

### React 18 features actually in use

- **Automatic batching** — came free with the `createRoot` migration, no
  code changes required. State updates that happen after an `await` (the
  approve/cancel actions in `JobDetailDrawer` and `JobsList`, the per-row
  loop in `BulkImport`'s `validate()`) now collapse into a single re-render
  instead of one per `setState` call, which wasn't true under React 17.
- **`useDeferredValue`** — the dashboard search box in
  [src/components/Jobs/JobsList.tsx](./src/components/Jobs/JobsList.tsx)
  stays instantly responsive to typing while the filtered-grid recompute is
  allowed to lag a frame behind under load; a small spinner and a dimmed
  grid signal that it's catching up.
- **`useTransition`** — tab switches in the app shell
  ([src/components/App.tsx](./src/components/App.tsx)) mark the new tab
  selected immediately while mounting its content (a wizard, a `DataGrid`)
  gets deprioritized, with a thin progress bar under the tab list while
  that's pending. `BulkImport`'s per-row validation loop uses the standalone
  `startTransition` the same way — the progress counter updates on every
  row, un-throttled, while the row table re-render (up to `bulkRowLimit`
  rows) is deferred so it never makes the counter itself feel stuck.
- **Not adopted: Suspense-based data fetching.** TanStack Query v4's
  per-query `suspense: true` is still experimental, and turning it on here
  would mean restructuring every `DataState` loading branch and adding error
  boundaries app-wide — a bigger change than "nice upgrade" justifies right
  now. Worth revisiting if the app ever moves to TanStack Query v5.
